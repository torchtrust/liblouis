 ...
 */
int
stringHash (const widechar * c, int length, int *lengthUsed, int mode)
{
  ...
  if (c != NULL)
      ...
      else
	{
      ...
      if (lengthx > 1)
	for (k = 0; k < lengthx; k++)
	  saved[k] = findCharOrDots (c[k], mode)->lowercase;
    }
  else
    lengthx--;
  if (lengthUsed != NULL)
...
passSelectRule ()
{
  ...
  if (findAttribOrSwapRules ())
    return;
  hashBucket = stringHash (&currentInput[src], length, &hashLength, 1);
  dots = findCharOrDots (currentInput[src], 1);
  do
    {
      if (hashLength > 1)
	ruleOffset = table->forRules[hashBucket];
      ...
      while (ruleOffset)
	{
	  ...
	  transOpcode = transRule->opcode;
	  transCharslen = transRule->charslen;
	  if (((transCharslen <= length) && checkDots ()))
	    switch (transOpcode)
      ...
      hashBucket = stringHash (NULL, 0, &hashLength, 1);
    }
  while (hashLength >= 0);
}
...
static int putCharacter (widechar character);
static int translateString ();
static int compbrlStart = 0;
static int compbrlEnd = 0;

/**
 * This function is used by other modules to set the table variable so 
 * they can use functions in this module that depend only on the table.
 */
const TranslationTableHeader *
setTable (const char *tableList)
{
  return (table = lou_getTable (tableList));
}
...
* therefore if we want the complete word its: wordEnd-wordStart+1 */
  if (!hyphenate (&currentInput[wordStart], wordEnd - wordStart + 1, hyphens))
    return 0;
...
/*check for valid Translations. Return value is in transRule. */
  ...
  int k;
  TranslationTableOffset ruleOffset;
  int hashBucket = stringHash (&currentInput[src], length, 
  &hashLength, 0);
  do
    {
	if (hashLength > 1)
	...
	{
	  curCharDef = findCharOrDots (currentInput[src], 
	  0);
      ...
      while (ruleOffset)
	{
	  ...
	  transOpcode = transRule->opcode;
	  transCharslen = transRule->charslen;
	  if ((transCharslen <= length) && validMatch ())
	    {
	      /* check this rule */
...
/*Done with checking this rule */
      ...
      hashBucket = stringHash (NULL, 0, &hashLength, 0);
    }
  while (hashLength >= 0);
}
